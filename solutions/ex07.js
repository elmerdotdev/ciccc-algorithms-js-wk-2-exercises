...
const checkAir = function (samples, threshold) {
  // Use reduce to count the number of dirty samples
  const dirtyCount = samples.reduce((count, sample) => {
    return sample === 'dirty' ? count + 1 : count;
  }, 0);

  // Calculate the proportion of dirty samples
  const proportionDirty = dirtyCount / samples.length;

  // Determine if the air is polluted or clean
  return proportionDirty > threshold ? 'Polluted' : 'Clean';
};
