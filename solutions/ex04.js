...
const instructorWithLongestName = function (instructors) {
  // Initialize a variable to keep track of the instructor with the longest name
  let longestNameInstructor = instructors[0];

  // Iterate through the array of instructors
  instructors.forEach(instructor => {
    if (instructor.name.length > longestNameInstructor.name.length) {
      longestNameInstructor = instructor; // Update the longest name instructor
    }
  });
